...
float k = 0, j = 0.1f, l = 1;  //k is to increase the amount of the gradient of the splash screen circle,   j will increase the size of the splash screen arrow, and l will keep track of arrow direction
...
public void splashScreen() {
  ...
  if (j <= 10) {
    ...
    popMatrix();
    l += 0.25f;  //Increment arrow size and rotation
    j += (j+ 0.1f) * (j/5);
  } 
  else {  //Randomize colour
    colourArrow = PApplet.parseInt (random(3)+1);
    j = 0.8f;
  }
...
public void userInput() {  //Receives and interprets keyboard arrows and turns into an integer
  if (keyPressed) {
    if (key == CODED) {
      if (keyCode == UP) {
        response = 1;
  ...
  if (mode == '5') {  //If the user wants to see instructions
    ...
    if (dir == 0 && assignment == true) {  //If variables havent been assigned
      ...
      if (mode == '1') {  //Depending on the mode they chose, randomizes the colour      (Easy)
        ...
        colourArrow = PApplet.parseInt (random(2) + 1);
      } 
...
public void draw() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "SKYFLARESTUDIOS_Arrows" };
    if (passedArgs != null) {
